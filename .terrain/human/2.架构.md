...
| 数据契约集中 | `models.rs` 定义 `Position`/`Transaction`/`FearGreedSnapshot` | 模块间传递数据的"通用语"，跨层不私传 |
...
| 数据模型 | `src/models.rs` | 持仓/交易/快照数据契约 + 收益计算 | `Position` / `Transaction` / `FearGreedSnapshot` |
...
**价格历史的"为未来铺路"**：`price_history` 表（`src/db.rs:52`）单独累积每日价格，注释明确写着这是为趋势锚（价格 vs 12 月均线）准备的（`src/db.rs:300-302`）。架构允许"现在的信号用情绪、未来的信号用趋势"平滑演进。
