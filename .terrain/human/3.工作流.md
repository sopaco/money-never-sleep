...
flowchart TD
    ...
    L6 -->|是| L8["db.update_price<br/>更新 positions + price_history"]
    L5 --> L6
    L7 --> L2
    L8 --> End([更新汇总展示])
```
...
sequenceDiagram
    ...
    loop 每个标的
        ...
        Q-->>M: PriceUpdate / 警告
        M->>D: update_price(更新 current_price + price_history)
    end
    M-->>U: 更新汇总（新/旧价、来源）
```
...
| 落库 | db.rs | `PriceUpdate` | 无 | 同时更新 `positions.current_price` 与 `price_history` |
