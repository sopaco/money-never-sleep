...
五张表对应 `models.rs` 的三类业务实体 + 两个"为功能演进预留"的数据累积表：`positions`/`transactions` 对应持仓与流水，`fear_greed_snapshots` 对应恐贪快照，而 `cash` 是单行现金，`price_history` 是为趋势锚信号预埋的价格序列。
...
erDiagram
    ...
    POSITIONS ||--o{ PRICE_HISTORY : "一码多日价格"
```
...
为趋势锚信号预埋的价格累积表，`UNIQUE(asset_code, price_date)` 保证同日只留一条（upsert，`record_price`，`src/db.rs:303`）。注释明确写着它的设计意图：老表只存 `current_price` 无法回溯历史（`src/db.rs:300-302`），趋势锚（价格 vs 12 月均线）需要历史序列，此表从今日起就开始累积。
...
| asset_code | TEXT | NN | 标的代码 |
| price | REAL | NN | 当日价格 |
| price_date | TEXT | NN | 日期 |

月末序列接口：`monthly_price_history`（`src/db.rs:319`）。

---

### `fear_greed_snapshots`（恐贪快照）

每日一条，同日先删后插保证唯一（`save_fear_greed_snapshot`，`src/db.rs:373`）。4 个历史参照值来自 CNN 接口，供风险警告（`strategy::check_risk_warnings`）使用。
