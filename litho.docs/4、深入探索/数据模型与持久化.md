...
**数据模型与持久化**是 mns（Market Neutral Strategist）系统中最关键的基础设施模块之一，承担着**金融数据结构定义、业务状态存储、事务一致性保障与审计追溯支持**的四大核心职责。该模块由 `models.rs`（金融数据模型）与 `db.rs`（数据库操作）两个子模块协同构成，共同构建了系统“**状态即数据、决策即计算、执行即事务**”的底层数据基石。
