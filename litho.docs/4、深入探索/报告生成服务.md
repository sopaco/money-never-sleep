...
**报告生成服务**（Report Generation Service）是 **mns（Market Neutral Strategist）** 系统的核心业务模块之一，承担将量化策略引擎输出的结构化决策信号，转化为人类可读、具备执行指导意义的中文投资日报的职责。该服务不参与策略计算或数据持久化，而是作为**决策成果的聚合器与表达层**，实现从“机器决策”到“人类执行”的关键转化。
