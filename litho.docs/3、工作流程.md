...
本系统（mns，Market Neutral Strategist）是一个面向个人逆向投资者的本地命令行投资辅助工具，其核心价值在于通过**数据驱动的纪律性交易策略**，将市场情绪（CNN Fear & Greed Index）、持仓表现与用户自定义规则相结合，自动生成可执行的投资日报，从而降低情绪化交易风险，提升长期投资回报的稳定性。
...
| **系统初始化与配置管理流程** | `mns init` / `mns config` | 建立系统运行基础，确保规则合法 | 可验证的配置文件（config.toml）与数据库结构 |
