...
name: mns-cli
description: |
  MNS (Market Neutral Strategist) CLI skill for autonomous agents. Provides investment
  portfolio management, automated trading strategy execution, sentiment analysis using
  CNN Fear & Greed Index, and report generation. Use when managing a contrarian investment
  portfolio, executing buy/sell decisions based on market sentiment thresholds, or
  generating daily strategy reports.

  Triggers include: "管理投资组合", "执行交易策略", "获取市场报告", "查看持仓收益",
...
2. **交易执行**: 记录买入/卖出操作，更新资产当前价格
3. **策略报告**: 自动生成基于最新恐贪指数的买卖建议
...
npx @mns/cli-darwin-arm64 init  # macOS Apple Silicon
# 或
npx @mns/cli-linux-x64 init     # Linux
# 或
npx @mns/cli-win-x64 init       # Windows
...
npx @mns/cli-darwin-arm64 buy SH600000 500 12.30
```
...
```bash
# 查看当前持仓（含年化收益）
npx @mns/cli-darwin-arm64 portfolio
...
```bash
# 更新资产当前价格
npx @mns/cli-darwin-arm64 price QQQ 460.00
...
npx @mns/cli-darwin-arm64 config sell_ratio.("greed","between") 0.20
```
...
# 2. 根据建议执行买入（如有）
npx @mns/cli-darwin-arm64 buy QQQ 50 445.00

# 3. 更新价格（如有新价格）
npx @mns/cli-darwin-arm64 price QQQ 448.50

# 4. 记录卖出（如有）
npx @mns/cli-darwin-arm64 sell QQQ 100 455.00
...
```bash
# 查看当前买入比例
npx @mns/cli-darwin-arm64 config buy_ratio
...
```bash
# 1. 添加资产到池子
npx @mns/cli-darwin-arm64 add TSLA "特斯拉" us_stocks

# 2. 初始建仓买入
npx @mns/cli-darwin-arm64 buy TSLA 50 250.00

# 3. 记录当前价格（后续需要定期更新）
npx @mns/cli-darwin-arm64 price TSLA 255.00
```
...
本 skill 遵循 [Agent Skills Specification](https://agentskills.io/specification) v1.0 标准。Binary distribution 通过 npm 发布，agent 可通过 `npx @mns/cli-<platform>` 直接使用，无需本地安装。