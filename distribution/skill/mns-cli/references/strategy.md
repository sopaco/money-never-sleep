...
本文档详细说明 MNS 逆向投资策略的核心参数，包括阈值定义、买入/卖出比例配置、止盈线设置等。这些参数通过 `~/.mns/config.toml` 管理，可通过 `mns config` 命令动态调整。
...
npx @mns/cli-darwin-arm64 config thresholds.greed 70

# 将极度恐慌阈值从 25 提升至 30，减少极端情况下的买入频率
npx @mns/cli-darwin-arm64 config thresholds.extreme_fear 30
```
...
npx @mns/cli-darwin-arm64 config buy_ratio.extreme_greed 0.10

# 策略2: 中性区间也暂停买入，只在明确的恐慌区间出手
npx @mns/cli-darwin-arm64 config buy_ratio.neutral 0.00

# 策略3: 极端恐慌时全仓出击
npx @mns/cli-darwin-arm64 config buy_ratio.extreme_fear 0.70
```
...
```bash
# 极度贪婪且大幅盈利时，卖出 70%
npx @mns/cli-darwin-arm64 config sell_ratio.("extreme_greed","above_high") 0.70

# 贪婪区间小幅盈利时也卖出 30%
npx @mns/cli-darwin-arm64 config sell_ratio.("greed","between") 0.30

# 中性区间也开始部分止盈（保守策略）
npx @mns/cli-darwin-arm64 config sell_ratio.("neutral","above_high") 0.20
```
...
```bash
# 保守止盈：年化 8% 就开始卖
npx @mns/cli-darwin-arm64 config annualized_target_low 8.0
npx @mns/cli-darwin-arm64 config annualized_target_high 12.0

# 激进止盈：年化 20% 才开始卖
npx @mns/cli-darwin-arm64 config annualized_target_low 15.0
npx @mns/cli-darwin-arm64 config annualized_target_high 25.0
```
...
npx @mns/cli-darwin-arm64 config sell_ratio
npx @mns/cli-darwin-arm64 config thresholds
```
...
```bash
npx @mns/cli-darwin-arm64 config > config_backup_2025-06-16.toml
```
...
# 访问 sell_ratio 的元组键
npx @mns/cli-darwin-arm64 config sell_ratio.("greed","above_high")
```
