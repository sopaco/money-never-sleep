...
- **`mns report` 实盘走的是"情绪锚"（纯按恐贪指数映射目标仓位），不是 `mns backtest` 默认展示的
  "趋势锚"（价格 vs 12 月均线 + 情绪倾斜）。** 二者样本内表现差很多——情绪锚年化 9.79%/Calmar
  1.07，趋势锚年化 12.75%/Calmar 1.17——**转述回测结论前先确认在说哪一种口径**，不要把趋势锚的
  数字当成 `mns report` 会给出的效果转述给用户。
- 情绪锚在样本内**风险调整后**优于买入持有（最大回撤约 9.2% vs 14.9%，Calmar 1.07 vs 0.98），
  但 `mns backtest validate` 的样本外（walk-forward）验证显示**这个优势不稳健**：样本外买入
  持有在**收益和 Calmar 上同时反超**情绪锚与趋势锚两种配置（买入持有 12.23%/Calmar 1.02，
  情绪锚仅 7.91%/Calmar 0.86，趋势锚 10.79%/Calmar 0.94）。bootstrap 分布显示这个差距落在
  统计噪声范围内，不具备显著性。
- `mns backtest validate` 还会把 2018、2022（数据集内仅有的两段明显下跌年份）逐年拆开单独展示：
  两种锚点在这两年确实比买入持有回撤更小，但这**不能替代 2000–2002／2008 这类多年期熊市压力
  测试**，那类数据当前不可得，而"熊市回撤保护"正是该策略的主要卖点，该卖点仍是未充分验证的
  claim。
- 回测区间是美股强牛市 + 长期低利率，结论不应外推到其他环境。

跑 `mns backtest validate` 可自行复现以上全部数字（不要只跑不带 `validate` 的 `mns backtest`，
那只展示样本内的趋势锚结果，会漏掉上面最重要的样本外反转结论）。**不要**把回测年化当作对未来
收益的预期转述给用户。
