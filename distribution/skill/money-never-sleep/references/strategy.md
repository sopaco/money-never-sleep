...
> ⚠️ **本文件描述的是已被取代的策略框架（v0.6.0 起）**
> `buy_ratio`（买入=可用现金的百分比）与 `sell_ratio`（卖出=持仓份额的百分比）
> 已不再驱动 `mns report`，仅保留用于 `mns backtest` 的"旧框架(现金比例)"对照。
>
> 现行框架为**目标仓位 + 偏离带**：情绪 → 风险资产目标总权重 → 与实际权重比较 →
> 偏离超带宽才动作。相关参数为 `target_weight.*` / `rebalance.*` / `costs.*`，
> 见 `SKILL.md` §2 与 `mns backtest params`。
